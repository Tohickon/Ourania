...
public class SwissData implements java.io.Serializable {
	...
	public String ODEGREE_STRING = "ï¿½ï¿½"; /* degree as string, utf8 encoding */
	...
	public static final String DIR_GLUE = (System.getProperty("file.separator") == null ? "/"
			: System.getProperty("file.separator")); // glue string for
														// directory/file
	/**
	 * Characters that separate one ephemeris directory from the next in a path list.
	 *
	 * <b>Was ";:" - both - and on Windows that silently cut every absolute path in half at
	 * the drive letter.</b> "C:/Users/daver/.../assets" was parsed as the two paths "C" and
	 * "/Users/daver/.../assets", which is visible in any swe_calc error text this app
	 * produces: "not found in the paths of: 'C', '/Users/...'". It has worked to date only
	 * because the second fragment resolves against the current drive and everything here
	 * lives on C:. Move the project to another drive and every ephemeris file disappears -
	 * the planets would fall back to Moshier without comment and the four asteroid files
	 * would start returning rc=-1, which reads as data corruption rather than a path bug.
	 *
	 * Upstream C uses ';' on Windows and ':' on Unix. DIR_GLUE two lines above already asks
	 * the platform for its separator; this now does the same, which is the consistency the
	 * port was missing rather than a new policy.
	 *
	 * Measured 2026-08-27: with ";:" the path split into 2 entries, with this it is 1.
	 */
